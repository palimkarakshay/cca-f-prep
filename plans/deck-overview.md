...
Three mechanisms, each grounded in peer-reviewed cognitive-psychology
research:

- **Spaced retrieval keeps yesterday's lesson alive.** Expanding-interval
  practice (1/3/7/14/30d) produces ~2× retention vs massed practice
  *(spacing effect — Cepeda et al. 2008, Psych Sci; 254-study meta in
  Cepeda 2006)*. Without review, retention plateaus near 25% by day 6
  *(Murre & Dros 2015, replicating Ebbinghaus 1885)*.
- **Streaks + variable rewards convert intent into habit.** Hook Model
  (trigger → action → variable reward → investment) plus loss-aversion
  via streak; Duolingo precedent — CURR +21%, DAU 4.5× over 4 years
  *(Eyal 2014; Mazal 2022)*.
- **Cohort surface (peer + accountability) drives 75–96% completion vs
  3–10% MOOC.** Maven W1→W2 retention 96% vs MOOC 16%; altMBA 96%
  completion *(Maven; Reich & Ruipérez-Valiente 2019, Science)*.
...
## Why SMEs ship better content

Three scaffolds, each fixing a known SME failure mode:

- **CTA-style intake captures the 70% of decisions experts forget to
  mention.** Cognitive Task Analysis 5-probe intake (novice-error /
  one-principle / worked-example pair / boundary case / nearest
  confusable) *(Clark, Feldon, van Merriënboer, Yates & Early; Lee 2004:
  +46% post-training learning gain, Cohen's d ≈ 1.72)*.
- **Backward-design template forces "what's the test?" before "what's the
  lesson?"** *(Wiggins & McTighe 1998 — Understanding by Design)*.
- **Critic flags expert blind spots before publish.** SMEs systematically
  over-estimate novice prerequisites *(Nathan & Petrosino 2003 — expert
  blind spot; Hinds 1999 — curse of expertise)*.

The premise: SMEs aren't bad teachers. They have specific cognitive
artefacts that don't transfer without specific elicitation tooling.

---

## Problems we mitigate, and how

The chain reads left-to-right: identified problem → specific feature in
the platform → research source. Same matrix elaborated in
`content-pack-management-plan.md` §D0 and mirrored in every deck.

**Learner retention + absorption:**

| Problem | Platform feature | Source |
|---|---|---|
| ~25% knowledge retained at day 6 without review | Spaced-review banner on the dashboard with one-tap "Start review"; expanding interval 1/3/7/14/30d | Cepeda 2008; Murre & Dros 2015 |
| Streaks computed but no return-trigger | Streak counter + web-push at user's modal study-time; streak-freeze | Eyal 2014; Mazal 2022 (Duolingo CURR +21%) |
| Re-reading produces fluency illusion, not learning | Mid-lesson retrieval gate (recall before reveal); write-the-principle field | Roediger & Karpicke 2006; Slamecka & Graf 1978 |
| No metacognitive calibration capture | JOL slider before reveal; calibration-Δ trend visible | Dunlosky & Bjork; Kruger & Dunning 1999 |
| Blocked practice transfers ~30% worse | Recommender enforces sub-area rotation per session | Rohrer & Taylor 2007 |
| No social/cohort surface; SDT relatedness unmet | Per-cohort routes + peer-comparison + group leaderboard | Ryan & Deci; Maven 96% vs MOOC 16% |

**SME knowledge-sharing:**

| Problem | Platform feature | Source |
|---|---|---|
| SMEs omit ~70% of decisions when self-narrating | 5-probe `DrafterIntake` schema with required JSON fields | Clark / Feldon; Lee 2004 (+46% gain, d ≈ 1.72) |
| "Telling what I know" instead of designing toward an assessment | Backward-design "write the test first" mode; lesson editor locked until assessment authored | Wiggins & McTighe 1998 |
| Curse of expertise — under-explained bridging steps | "What would a novice get wrong?" probe required at submission | Nathan & Petrosino 2003 |
| Tacit knowledge unrecoverable from text-from-blank | Voice-first / camera-first authoring with auto-draft | Polanyi 1966; CTA-for-skilled-trades |
| Critic feedback is destination, not loop | Per-SME blind-spot dashboard aggregating feedback over time | Ericsson 1993 (deliberate-practice feedback channel) |

Every mitigation is a shipped Phase-2 surface — not exhortation, not a
"best-practice guideline." Outcome targets per Kirkpatrick L1–L4 in
`content-pack-management-plan.md` §D4. Full evidentiary basis in
[`research-and-strategy-dossier.md`](./research-and-strategy-dossier.md)
— problem catalogue, mechanism set, industry case studies, per-problem
falsification triggers, and commercial viability.
...
- **80% of the global workforce is deskless** (~2.7B workers — TalentCards).
  ...
  field-services in one shot.
- **Compliance verticals (finance, healthcare, government) need
  audit-trail + version-pin + role-based publish** — same critic engine,
  different governance gates. <70% completion ⇒ 3.5× violation rate
  (KPI Depot 2026).
- **High-power-distance markets** (East Asia, LATAM, MENA, India) **need
  leader-endorsed pathways** — same SME scaffold, different framing.
- **Single-SME-bottleneck SMBs need succession-of-knowledge tooling** —
  same CTA pipeline, different urgency (bus-factor 1).

One engine, calibrated per segment, not eight rebuilds.
